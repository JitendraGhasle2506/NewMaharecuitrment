...
@Entity
@Table(
        name = "recruitment_request_sequence",
        uniqueConstraints = {
                @UniqueConstraint(
                        name = "uk_recruitment_request_sequence_date_type",
                        columnNames = { "sequence_date", "request_type_code" })
        },
        indexes = {
                @Index(
                        name = "idx_recruitment_request_sequence_date_type",
                        columnList = "sequence_date,request_type_code")
        })
@Getter
@Setter
@NoArgsConstructor
@AllArgsConstructor
public class RecruitmentRequestSequenceEntity {

    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    @Column(name = "recruitment_request_sequence_id")
    private Long recruitmentRequestSequenceId;
