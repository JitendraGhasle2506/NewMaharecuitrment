...
public interface EmployeeRepository extends JpaRepository<EmployeeEntity, Long> {
    ...
    @Query(value = """
            ...
            where upper(trim(coalesce(employee.status, ''))) = 'ACTIVE'
              ...
              and not exists (
                    select mapping.employeeProjectMappingId
            ...
            where upper(trim(coalesce(employee.status, ''))) = 'ACTIVE'
              ...
              and (:searchPattern is null
                   ...
                   or exists (
                        ...
                        where projectMapping.employee = employee
                          and (upper(coalesce(project.projectName, '')) like :searchPattern
                               or upper(coalesce(project.projectCode, '')) like :searchPattern)
                   ))
            """,
            countQuery = """
                    select count(employee)
                    ...
                    """)
    Page<EmployeeEntity> findActiveOnboardedWithProjectMapping(
            @Param("recruitmentType") String recruitmentType,
            @Param("searchPattern") String searchPattern,
            Pageable pageable);
    ...
    @Query(value = "select employee.employeeId as employeeId, "
            ...
            + "left join employee.designation designation "
            + "left join EmployeeCellMappingEntity cellMapping "
            + "on cellMapping.employee.employeeId = employee.employeeId "
            + "left join cellMapping.cell cell "
            + "left join EmployeeReportingMappingEntity reporting "
            + "on reporting.employeeId = employee.employeeId "
            + "and reporting.mappingId = (select max(latestReporting.mappingId) "
            + "from EmployeeReportingMappingEntity latestReporting "
            ...
            countQuery = "select count(employee) "
                    ...
                    + "or upper(coalesce(employee.fullName, '')) like :searchPattern "
                    + "or upper(coalesce(employee.email, '')) like :searchPattern "
                    + "or exists (select searchCellMapping.employeeCellMappingId "
                    + "from EmployeeCellMappingEntity searchCellMapping "
                    + "join searchCellMapping.cell searchCell "
                    + "where searchCellMapping.employee.employeeId = employee.employeeId "
                    + "and upper(coalesce(searchCell.cellName, '')) like :searchPattern) "
                    + "or exists (select searchReporting.mappingId "
                    + "from EmployeeReportingMappingEntity searchReporting "
                    + "left join EmployeeEntity searchManager "
                    + "on searchManager.employeeId = searchReporting.managerEmployeeId "
                    + "left join User searchReportingAuthority "
                    + "on searchReportingAuthority.id = searchReporting.hodUserId "
                    + "where searchReporting.employeeId = employee.employeeId "
                    + "and searchReporting.mappingId = (select max(latestSearchReporting.mappingId) "
                    + "from EmployeeReportingMappingEntity latestSearchReporting "
                    + "where latestSearchReporting.employeeId = employee.employeeId) "
                    + "and upper(coalesce(case when searchManager.employeeId is not null "
                    + "then searchManager.fullName else searchReportingAuthority.name end, '')) "
                    + "like :searchPattern) "
                    + "or (not exists (select existingReporting.mappingId "
                    + "from EmployeeReportingMappingEntity existingReporting "
                    + "where existingReporting.employeeId = employee.employeeId) "
                    + "and exists (select searchCellAuthorityMapping.mappingId "
                    + "from EmployeeCellMappingEntity authorityCellMapping "
                    + "join authorityCellMapping.cell authorityCell "
                    + "join CellReportingAuthorityMappingEntity searchCellAuthorityMapping "
                    + "on searchCellAuthorityMapping.cell.cellId = authorityCell.cellId "
                    + "join User searchCellAuthority "
                    + "on searchCellAuthority.id = searchCellAuthorityMapping.authorityUserId "
                    + "where authorityCellMapping.employee.employeeId = employee.employeeId "
                    + "and upper(coalesce(searchCellAuthority.name, '')) like :searchPattern)))")
