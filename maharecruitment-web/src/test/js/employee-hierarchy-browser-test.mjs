...
const server = createServer(async (request, response) => {
    ...
    try {
        if (url.pathname.startsWith('/api/')) {
            ...
            else data = node(Number(url.searchParams.get('nodeId') || 1), true);
...
try {
    ...
    await evaluate(`document.getElementById('ehSearch').value='EMP006'; document.getElementById('ehSearchForm').requestSubmit()`);
    ...
    console.log(`PASS: expand/collapse, connectors, non-overlap, search, branch merging, zoom, desktop/tablet/mobile. Screenshots: ${artifacts}`);
} finally {
