...
public class V50__pre_onboarding_onboarding_date_optional extends BaseJavaMigration {

    @Override
    public void migrate(Context context) {
        Connection connection = context.getConnection();
        if (!tableExists(connection, "agency_candidate_pre_onboarding")
                || !columnExists(connection, "agency_candidate_pre_onboarding", "onboarding_date")) {
            return;
        }
        ...
        jdbcTemplate.execute(
                "alter table agency_candidate_pre_onboarding "
                        + "alter column onboarding_date drop not null");
    }
