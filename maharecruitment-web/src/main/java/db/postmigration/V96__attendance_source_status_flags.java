    ...
    public void migrate(Context context) throws Exception {
        ...
        jdbcTemplate.execute("""
                ...
                set mobile_app_status = case
                        when check_in_time is not null or check_out_time is not null then 'Y'
                        else 'N'
                    end,
                    api_status = case
                        when nullif(trim(coalesce(in_time, '')), '') is not null
                          or nullif(trim(coalesce(out_time, '')), '') is not null then 'Y'
                        else 'N'
                    end
                """);
    }
