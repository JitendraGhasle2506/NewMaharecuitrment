...
public class R__hr_employee_project_mapping_navigation extends BaseJavaMigration {

    private static final String ROLE_HR = "ROLE_HR";
    private static final String MENU_NAME = "Employee Project Mapping";
    private static final String MENU_URL = "/hr/employee-project-mappings";
    private static final String MENU_ICON = "fa-solid fa-diagram-project";
        ...
        if (menuId == null) {
            jdbc.update(
                    "insert into mst_menu (menu_name_english, menu_name_marathi, is_active, icon, url, is_sub_menu) "
                            + "values (?, ?, 'Y', ?, ?, 1)",
                    MENU_NAME, MENU_NAME, MENU_ICON, MENU_URL);
            menuId = jdbc.query(
                    "select menu_id from mst_menu where upper(url) = upper(?) fetch first 1 row only",
                    rs -> rs.next() ? rs.getLong(1) : null,
                    MENU_URL);
            ...
            jdbc.update(
                    ...
                    MENU_NAME, MENU_NAME, MENU_ICON, MENU_URL, menuId);
        }
        if (menuId != null) {
            ...
            if (count != null && count == 0) {
                jdbc.update("insert into menu_role (menu_id, id) values (?, ?)", menuId, roleId);
            }
        }
    }
